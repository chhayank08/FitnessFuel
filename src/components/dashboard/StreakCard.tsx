...
const StreakCard: React.FC<StreakCardProps> = ({ streak, className = '' }) => (
  <Card className={`p-5 ${className}`}>
    <div className="flex items-center justify-between">
      ...
      <Flame className={`h-4 w-4 ${streak.current > 0 ? 'text-success-400' : 'text-gray-500'}`} />
    </div>

    <p className="mt-3 font-display text-2xl font-semibold text-white">
      <AnimatedNumber value={streak.current} />
      <span className="ml-1 text-sm font-normal text-gray-400">{streak.current === 1 ? 'day' : 'days'}</span>
    </p>
      ...
      {streak.last7.map((active, i) => {
        ...
        return (
          <div key={i} className="flex flex-col items-center gap-1.5">
            ...
            />
            <span className="text-[10px] text-gray-500">{DAY_LETTERS[date.getDay()]}</span>
          </div>
        );
      })}
    </div>
